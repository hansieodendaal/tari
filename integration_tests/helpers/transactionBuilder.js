...
class TransactionBuilder {
  ...
  buildSenderMetaChallenge(script, features, scriptOffsetPublicKey, publicNonce) {
    const KEY = null; // optional key
    const OUTPUT_LENGTH = 32; // bytes
    const context = blake2bInit(OUTPUT_LENGTH, KEY);
  ...
  addOutput(amount) {
    ...
    let public_nonce = this.kv.public_key("common_nonce");
    let private_nonce = this.kv.private_key("common_nonce");
    let sender_meta_challenge = this.buildSenderMetaChallenge(nopScriptBytes, outputFeatures, scriptOffsetPublicKey, public_nonce);
  ...
  generateCoinbase(value, privateKey, fee, lockHeight) {
    ...
    let sender_meta_challenge = this.buildSenderMetaChallenge(nopScriptBytes, outputFeatures, scriptOffsetPublicKey, sender_sig_public_nonce);
    let sender_sig = tari_crypto.sign_challenge_with_nonce(
      scriptOffsetPrivateKey.toString("hex"),
